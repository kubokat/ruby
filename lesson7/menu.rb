  ...
  def print_stations_list
    @stations.each_with_index do |station, index|
      ...
      station.train_list do |train|
        puts "  Train number: #{train.number}, train type: #{train.class}, numbers of wagons: #{train.wagons.length}"
        i = 1
        train.wagons_list do |wagon|
          wagon_list = "     Wagon number:#{i} , wagon type: #{wagon.class}"

          if wagon.class == PassengerWagon
            wagon_list += ", free seats: #{wagon.free}, seats: #{wagon.number_of_seats}"
          else
            wagon_list += ", free value: #{wagon.free}, volume: #{wagon.volume}"
          end
          i += 1
          puts wagon_list
        end
